...
void Octree::build(std::vector<Triangle> *triangles, float minx, float miny, float minz, float maxx, float maxy, float maxz) {
	...
	//for ( int i = 0; i < 8; i++ )
	buildRec(triangles, &vox, &vminx, &vminy, &vminz, &vmaxx, &vmaxy, &vmaxz);
	for ( int i = 0; i < 8; i++ )
		std::cout << i << ": " << vox[i]->size() << "\n";

	voxels = &vox;
	for ( int i = 0; i < voxels->size(); i++ ) {
		std::cout << "i: " << i << " = " << (*voxels)[i]->size() << "\n";
	}
}

void Octree::buildRec(std::vector<Triangle> *triangles, std::vector<Voxel *> *vox, Vector *minx, Vector *miny, Vector *minz, Vector *maxx, Vector *maxy, Vector *maxz) {
	Vector middlex = Vector((minx[0] + (maxx[0] - minx[0])/2)[0], 0.0f, 0.0f);
	Vector middley = Vector(0.0f, (miny[1] + (maxy[1] - miny[1])/2)[1], 0.0f);
	Vector middlez = Vector(0.0f, 0.0f, (minz[2] + (maxz[2] - minz[2])/2)[2]);
